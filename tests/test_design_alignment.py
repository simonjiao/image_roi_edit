...
class DesignAlignmentTest(unittest.TestCase):
    def test_staged_design_declares_five_stage_gate(self) -> None:
        doc = (ROOT / "docs" / "staged_roi_pipeline_design.md").read_text(encoding="utf-8")
        self.assertIn("阶段门禁只有 5 个", doc)
    ...
    def test_architecture_boundaries_live_outside_main_checklist(self) -> None:
        boundaries = (ROOT / "docs" / "workflow_architecture_boundaries.md").read_text(encoding="utf-8")
        self.assertIn("架构边界", boundaries)
        self.assertIn("workflow_checklist.md", boundaries)
    ...
    def test_text_shape_gap_table_syncs_status_to_checklist_items(self) -> None:
        ...
        for row in rows:
            ...
            self.assertIn("workflow_checklist.md", row)
