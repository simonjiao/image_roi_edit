...
class AutoRoiEvidenceTest(unittest.TestCase):
    def test_auto_region_evidence_records_search_edit_field_and_old_value_quality(self) -> None:
        ...
        self.assertEqual(evidence["roi_geometry"]["search_area"], 1100)
        self.assertEqual(evidence["roi_geometry"]["edit_area"], 364)
        self.assertTrue(evidence["roi_geometry"]["search_area_gte_edit_area"])
        self.assertTrue(evidence["roi_geometry"]["edit_within_search_roi"])
        self.assertGreater(evidence["roi_geometry"]["search_protected_overlap_pixels"], 0)
        self.assertEqual(evidence["roi_geometry"]["edit_protected_overlap_pixels"], 0)
        self.assertTrue(evidence["roi_geometry"]["edit_avoids_protected_text"])
